var Yasiv = Yasiv || {};/*global Yasiv window*/
...
Yasiv.restClient = function(){
    return {
        ...
        put : function(address, data, callback) {
            ...
            $.post(address, {data : JSON.stringify(data)}, callback, 'json');
        },
...
Yasiv.importanceBuilder = function(graph) {
    return {
        get : function(){
            var orderedNodes = [];
...
Yasiv.facebookGraphBuilder = function(graph, FB, layout) {
    var MAX_CHUNK = 70, // 70*70 ≈ 5000 - the nearest to undocumented limit of records returned in one request.
    ...
    onFriendsGathered = function(response, startFromId, progress){
        ...
        var profiles = response.data,
            friendIds = profiles.map(function(u) { return u.uid; }),
            chunks = chunkArray(friendIds, MAX_CHUNK),
            work = [],
            rootNode = graph.getNode(startFromId),
            rootPos;
        if (rootNode) {
            rootPos = layout.getNodePosition(rootNode.id);
        } else {
            rootPos = {x : 0, y : 0};
        }
    ...
    getFriendsList = function(startFromId, progress) {
        FB.api('/fql', { q : 'SELECT uid, name, pic_square, friend_count,sex,profile_url, is_app_user FROM user WHERE uid = '+ startFromId + ' OR uid IN (SELECT uid2 FROM friend WHERE uid1 = '+ startFromId + ')' },
        function(response) {
    ...
    return {
        buildMyFriendsGraph : function() {
           var progress = Yasiv.progressToken();



           FB.api('/fql?q=SELECT uid, name, friend_count, sex,profile_url, pic_square FROM user WHERE uid = me() OR uid IN (SELECT uid2 FROM friend WHERE uid1 = me()) and is_app_user = 1', 'get', function(response){
      ...
      var graphics = Viva.Graph.View.webglGraphics(),
          ...
          showDetailedView = function(node) {
              ...
              if (person.is_app_user && !person.friendsKnown && isMyFriend){
                      actionLink
                        .text('Show ' + person.name + '\'s friends')
                        .show()
                        .unbind('click')
                        .click(function() {
                              showFriends(node);
                  ...
                  if (!node.friendRequestSent) {
                      actionLink
                        ...
                        .unbind('click')
                        .click(function() {
                             FB.ui({ method: 'friends.add', id: node.id },
                             function(r){
                                 if (r && r.action === true) {
        ...
        fbBuilder = Yasiv.facebookGraphBuilder(graph, FB, layout),
           ...
           $('.color_select').click(function(){
               $('.color_select').removeClass('selected_color');
                var colorCss = $(this).css('background-color'),
                    ...
                    r = parseInt(colorMatch[1], 10), g = parseInt(colorMatch[2], 10), b = parseInt(colorMatch[3], 10),
